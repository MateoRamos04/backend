...
public class Envio {
    ...
    public static Envio desdeCampos(String[] c) {
        ...
        if (modalidad.equals("NORMAL")) {
            return new Envio(c[0], c[1], bruto - embalaje);
        }
        if (modalidad.equals("PRIORITARIO")) {
            return new EnvioPrioritario(c[0], c[1], bruto - embalaje);
        }
        throw new IllegalArgumentException("Modalidad Incorrecta: " + modalidad);
    }
